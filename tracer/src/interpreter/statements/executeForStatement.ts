import type { ForStatement, BlockStatement, VariableDeclaration, ExpressionStatement } from '@babel/types';
...
export function executeForStatement(
  ...
  for (;;) {
    ...
    env.recorder.record({
      line: getNodeLine(node),
      type: 'loop_iteration',
      description: `Loop iteration ${iterationCount} started.`,
    });
  ...
  env.recorder.record({
    line: getNodeLine(node),
    type: 'loop_exit',
    description: `For loop exited after ${iterationCount} iteration(s).`,
  });
